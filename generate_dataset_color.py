...
skeleton_pth = './dataset_additional_info_synthetic_hands_color/'  # yy: path to save the generated dataset
...
for i, img in enumerate(sorted(os.listdir(img_folder_pth))[:SMAPLE_NUM]):
    ...
    pose_pair_joint_locations = np.array([-z_positions[p[0]] for p in POSE_PAIRS])
    ...
    img_c1, img_c2, img_c3 = np.zeros_like(colored_mh_img), np.zeros_like(colored_mh_img), np.zeros_like(one_channel)
    blur_level, THICKNESS = 5, 5
    color_c1 = [(255, 0, 0), (0, 255, 0), (0, 0, 255), (127, 127, 0), (0, 127, 127)]
    color_c2 = [(255, 0, 0), (0, 255, 0), (0, 0, 255), (127, 127, 0), (0, 127, 127)]
    for i in np.flip(np.argsort(pose_pair_joint_locations)):
        pair = POSE_PAIRS[i]
        color = color_c1[i // 4]
        partA = pair[0]
    ...
    updown_pair_joint_locations = np.array([ - z_positions[p[0]] for p in UP_DOWN_PAIRS])
    ...
    img_c1, img_c2, img_c3 = np.zeros_like(ori_img), np.zeros_like(ori_img), np.zeros_like(one_channel)
    blur_level, THICKNESS = 5, 5
    color_c1 = [(255, 0, 0), (0, 255, 0), (0, 0, 255), (127, 127, 0), (0, 127, 127)]
    color_c2 = [(255, 0, 0), (0, 255, 0), (0, 0, 255), (127, 127, 0), (0, 127, 127)]
    for i in np.flip(np.argsort(pose_pair_joint_locations)):
